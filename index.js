...
function newProject(project) {
  if (project) {
      ...
      log.error('正确命令例子：dm new myproject')
  }
}
 ...
 * @param {String} typeName 
 */
function deletePage(pageName, typeName) {
  const folderPath = path.join(SRCPATH, `src/${typeName}`)
  const configPath = path.join(folderPath, 'config.json')
  if (path.join(folderPath, pageName)) {
    shell.rm('-rf', path.join(folderPath, pageName))
    let configJSON = readJson(configPath)
    log.info(`删除项目${pageName}成功`)
    const paths = configJSON.path
    const index = paths.findIndex(item => item.src === `./${pageName}`)
    paths.splice(index, 1)
    if (paths.length === 0) {
      shell.rm('-rf', folderPath)
      log.info(`删除分类${typeName}成功`)
    } else {
      writeJson(configPath, configJSON)
    }
  }
